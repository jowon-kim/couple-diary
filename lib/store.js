...
export async function patchSettings(p) {
  ...
  try {
    rows = await sql`
      insert into settings (id, title, subtitle, theme, locale, region, name_a, name_b, since, show_milestones)
      values (
        1,
      ...
      returning title, subtitle, theme, locale, region, name_a, name_b, to_char(since,'YYYY-MM-DD') as since, show_milestones
    `;
  } catch (e) {
