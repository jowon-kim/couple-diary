...
export async function getSettings() {
  ...
  for (let i = 0; i < tries.length; i++) {
    try {
      ...
      if (i > 0) console.error('settings에 없는 컬럼이 있습니다 — schema.sql의 alter를 실행해 주세요');
...
export async function patchSettings(p) {
  ...
  try {
    rows = await sql`
      insert into settings (id, title, subtitle, theme, name_a, name_b, since, show_milestones)
      values (
        1,
        coalesce(${p.title ?? null}::text, '우리어리'),
        coalesce(${p.subtitle ?? null}::text, '우리 오늘 뭐하지'),
        coalesce(${p.theme ?? null}::text, '복숭아'),
        coalesce(${p.names?.a ?? null}::text, '나'),
        coalesce(${p.names?.b ?? null}::text, '너'),
        ${p.since ?? null}::date,
      ...
      returning title, subtitle, theme, name_a, name_b, to_char(since,'YYYY-MM-DD') as since, show_milestones
    `;
  } catch (e) {
    console.error('settings에 없는 컬럼이 있습니다 — schema.sql의 alter를 실행해 주세요', e.message);
    rows = await sql`
...
export async function listPhotoIds() {
  try {
    ...
    console.error('photos 조회 실패 (테이블이 없나요?)', e.message);
    return [];
  }
}
...
export async function listPushSubs() {
  try {
    ...
    console.error('push_subs 조회 실패 (테이블이 없나요?)', e.message);
    return [];
  }
}
